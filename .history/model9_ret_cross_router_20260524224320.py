...
class AdapterLinear(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        return self.mora(hidden_states, gate1, chu'l, fusion, result)
...
def _apply_for_layer(layer_module: nn.Module,
    ...
    def get_router(input_dim, tag: Optional[str] = None):
        ...
        if getattr(config, "disable_router1", False):
            router1 = None
        elif router1_input_source == "hidden_token":
        ...
        if router1 is not None and router1.task_router is not None:
