...
class RetrievalQueryProjector(nn.Module):
    ...
    def forward(self, ret_query: torch.Tensor) -> torch.Tensor:
        ...
        return ret_query.to(dtype=ret_query.dtype, device=ret_query.device)
...
class RetrievalAwareTokenRouter(nn.Module):
    ...
    def __init__(self, config: UmRaConfig, input_dim: int, layer_id: int,
        ...
        # Keep retrieval branch almost off at init; let training gradually turn it on.
        self.gate_init_logit = nn.Parameter(torch.tensor(-8.0, dtype=config.torch_dtype))
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        if ret_query is None:
            ...
            gamma = torch.sigmoid(self.gate(torch.cat([h_norm, ret_norm], dim=-1)) + self.gate_init_logit)
            router_input = (1.0 - gamma) * h_mean + gamma * r_tok
...
class MoRa(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        # 融合：w1*out1 + w2*out2
        w1 = fusion[:, 0].unsqueeze(-1).unsqueeze(-1).to(dtype=out1.dtype, device=out1.device)
        w2 = fusion[:, 1].unsqueeze(-1).unsqueeze(-1).to(dtype=out2.dtype, device=out2.device)
        output = w1 * out1 + w2 * out2 +  self.router1_shared_expert_weight * shared_out1 if self.router1_use_shared_expert else w1 * out1 + w2 * out2
