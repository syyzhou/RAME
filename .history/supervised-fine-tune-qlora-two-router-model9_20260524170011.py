...
class ConfigArguments:
    llama_compat: bool = field(
        default=False,
        metadata={"help": "Enable Llama base compatibility: Llama RoPE config and pad=eos token handling."},
    )
    ...
    trajectory_dim: int = field(
        default=0, metadata={"help": "轨迹向量维度。0表示自动从trajectory_embedding_path推断"}
    )
    ...
    router1_input_source: str = field(
        default="retquery",
        metadata={"help": "router1输入来源: retquery, hidden_mean 或 hidden_token"}
    )
...
def train():
    ...
    if config_args.router1_input_source not in {"retquery", "hidden_mean", "hidden_token"}:
        raise ValueError(
            "--router1_input_source must be 'retquery', 'hidden_mean', or 'hidden_token', "
    ...
    use_llama_compat = config_args.llama_compat or "llama" in model_args.model_name_or_path.lower()
    ...
    inferred_trajectory_dim = None
    if config_args.router2_input_source == "trajectory":
        train_ds = data_module["train_dataset"]
        if getattr(train_ds, "trajectory_embeddings", None) is not None:
            inferred_trajectory_dim = int(train_ds.trajectory_embeddings.shape[1])
    dataset_name = infer_dataset_name(data_args.dataset, data_args.dataset_name)
    poi_num = {'NYC': 4981, 'TKY': 7833, a': 9690}[dataset_name]
    ...
    peft_config = UmRaConfig(
        ...
        trajectory_dim=(
            int(config_args.trajectory_dim)
            if int(config_args.trajectory_dim) > 0
            else (inferred_trajectory_dim if inferred_trajectory_dim is not None else model.config.hidden_size)
        ),
