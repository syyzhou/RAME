...
class FusionGate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor,
        ...
        """
        hidden_states: (B, T, D) or (B, D)
        traj_embedding: (B, traj_dim) or (traj_dim,)
        return: (B, 2) sample-wise softmax weights
        """
...
class MoRa(nn.Module):
    def __init__(self, base_layer: nn.Linear, config: UmRaConfig):
        ...
        if self.disable_router1 and self.disable_router2:
            raise ValueError("disable_router1 and disable_router2 cannot both be True.")
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_cache:
            ...
            if self.disable_router1:
                ...
                # Fusion gate uses mean-pooled hidden states, not retrieval-aware router1 context.
