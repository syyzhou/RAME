...
def train():
    ...
    inferred_traj_dim = None
    train_ds = data_module["train_dataset"]
    if config_args.use_trajectory_routing and getattr(train_ds, "trajectory_embeddings", None) is not None:
        inferred_traj_dim = int(train_ds.trajectory_embeddings.shape[1])
    ...
    peft_config = UmRaConfig(
        ...
        trajectory_dim=(
            inferred_traj_dim
            if inferred_traj_dim is not None
