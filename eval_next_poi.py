...
def resolve_device(device_arg: str) -> str:
    ...
    if device_arg.startswith("cuda:") and torch.cuda.is_available():
        try:
            req_idx = int(device_arg.split(":")[1])
            visible_count = torch.cuda.device_count()
            if req_idx >= visible_count:
                print(f"[Warn] Requested {device_arg}, but only {visible_count} visible CUDA device(s). Fallback to cuda:0.")
                return "cuda:0"
        except Exception:
            print(f"[Warn] Invalid device format: {device_arg}. Fallback to auto.")
            return "cuda:0"
    return device_arg
