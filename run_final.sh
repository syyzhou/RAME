...
OUTPUT_DIR="${OUTPUT_ROOT}/${DATASET_NAME}-${RUN_TAG}"
THRESHOLD_MIB="${THRESHOLD_MIB:-10000}"
POLL_SECONDS="${POLL_SECONDS:-60}"
LOG_DIR="${LOG_DIR:-.${BASE_ROOT}/logs/${DATASET_NAME}-${RUN_TAG}}"
...
ROUTER_MODEL_FILE="${ROUTER_MODEL_FILE:-model9}"
...
# TRAIN_TRAJ_EMB="${TRAIN_TRAJ_EMB:-./datasets/${DATASET_NAME}/preprocessed/trajectory_embeddings_gsm8k_compact.pt}"
# TEST_TRAJ_EMB="${TEST_TRAJ_EMB:-./datasets/${DATASET_NAME}/preprocessed/test_embeddings_gsm8k_compact.pt}"
ROUTER2_INPUT_SOURCE="${ROUTER2_INPUT_SOURCE:-trajectory}"

# 检索 query 向量（由 export_fused_queries.py 导出）
# TRAIN_RET_QUERY="${TRAIN_RET_QUERY:-./datasets/${DATASET_NAME}/preprocessed/gsm8k_final_valartifact_fused_train_queries.pt}"
# TEST_RET_QUERY="${TEST_RET_QUERY:-./datasets/${DATASET_NAME}/preprocessed/gsm8k_final_valartifact_fused_test_queries.pt}"
TRAIN_RET_QUERY="${TRAIN_RET_QUERY:-./datasets/${DATASET_NAME}/preprocessed/gsm8k_ablation_fused_queries/full_train_fused_queries.pt}"
TEST_RET_QUERY="${TEST_RET_QUERY:-./datasets/${DATASET_NAME}/preprocessed/gsm8k_ablation_fused_queries/full_test_fused_queries.pt}"
...
if [[ "${ROUTER1_INPUT_SOURCE}" != "retquery" && "${ROUTER1_INPUT_SOURCE}" != "hidden_token" ]]; then
  echo "[$(timestamp)] ERROR: ROUTER1_INPUT_SOURCE must be retquery or hidden_token, got ${ROUTER1_INPUT_SOURCE}" | tee -a "${STATUS_LOG}" >&2
  exit 1
fi
...
if [[ "${ROUTER1_INPUT_SOURCE}" == "retquery" && -n "${TRAIN_RET_QUERY}" ]]; then
...
if [[ "${ROUTER1_INPUT_SOURCE}" == "retquery" ]]; then
...
if [[ "${ROUTER1_INPUT_SOURCE}" == "retquery" && -n "${TEST_RET_QUERY}" ]]; then
